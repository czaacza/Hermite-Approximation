...
make_matrix(int rn, int cn)
{
  ...
  if (new_mat == NULL)
    return NULL;
  if ((new_mat->e = calloc((size_t)rn * (size_t)cn, sizeof *new_mat->e)) == NULL) //alocate memory and initialize 0
  {
    free(new_mat);
    return NULL;
  }
...
void put_entry_matrix(matrix_t *m, int i, int j, double val)  //put val to matrix[i,j]
{
  if (i >= 0 && i < m->rn && j >= 0 && j <= m->cn)
    m->e[i * m->cn + j] = val;
}

void add_to_entry_matrix(matrix_t *m, int i, int j, double val) //add val to matrix[i,j]
{
  if (i >= 0 && i < m->rn && j >= 0 && j <= m->cn)
    m->e[i * m->cn + j] += val;
}
...
double
get_entry_matrix(matrix_t *m, int i, int j) //get value from matrix[i,j]
{
  if (i >= 0 && i < m->rn && j >= 0 && j <= m->cn)
    return m->e[i * m->cn + j];
  else
